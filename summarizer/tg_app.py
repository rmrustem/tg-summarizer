...
async def summarize(messages: list[Message], chat_id: int) -> str:
    ...
    prompt = f"""Напиши простым языком в шутливом тоне краткое резюме сообщений из Telegram чата (номер чата "{chat_id}") в виде связанного текста обсудили то, отметили это, сделали что-то. Сообщения на одну тему должны быть сгруппированы в один абзац, независимо от порядка их обсуждения. Отдельные слова резюме должны быть оформлены ссылками на исходные сообщения telegram в формате HTML. Не используй выделение с помощью `**`. Написание имен пользователей не изменяй и не переводи. В начале каждого блока вставь 1 emoji в тему сообщений в качестве буллитов. Вводная часть или заголовок не нужны. Длина текста должна быть строго меньше 3500 символов. Текст сообщений: \n\n"""  # pylint:disable=line-too-long
